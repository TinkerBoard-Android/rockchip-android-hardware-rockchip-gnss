...
static void nmea_reader_set_callback(NmeaReader* const r, GpsCallbacks* const cbs){
...
static int  get_svid(int prn, int sv_type) { 
...
static int nmea_reader_update_sv_status_gnss(NmeaReader* r, int sv_index,
...
static void nmea_reader_addc(NmeaReader* const r, int  c) {
    ...
    if (c == '\n') {
        r->in[r->pos + 1] = 0; // null terminate the strin
        nmea_reader_parse(r);
        DBG("start nmea_cb\n");
...
static void gps_state_start(GpsState*  s) {
    ...
    if (ret != 1)
        ERR("%s: could not send CMD_START command: ret=%d: %s", __FUNCTION__, ret, strerror(errno));
} 
 ...
 static void gps_state_stop(GpsState*  s) {
    ...
    if (ret != 1)
        ERR("%s: could not send CMD_STOP command: ret=%d: %s", __FUNCTION__, ret, strerror(errno));
} 

static void gps_state_restart(GpsState*  s) {
    ...
    if (ret != 1)
        ERR("%s: could not send CMD_RESTART command: ret=%d: %s",  __FUNCTION__, ret, strerror(errno));
}
...
static int rk_gps_init(GpsCallbacks* callbacks) {
...
static void rk_gps_cleanup(void) {
    GpsState*  s = _gps_state;
    if (s->init)
        gps_state_done(s);
    DBG("rk_gps_cleanup done");
}
...
static int rk_gps_inject_time(GpsUtcTime time, int64_t timeReference, int uncertainty) {
    return 0;
}

static int rk_gps_inject_location(double latitude, double longitude, float accuracy) {
    return 0;
}

static void rk_gps_delete_aiding_data(GpsAidingData flags) {
    return;
}

static int  rk_gps_set_position_mode(GpsPositionMode mode,
...
static const void* rk_gps_get_extension(const char* name) {
    DBG("rk_gps_get_extension name=[%s]\n", name);
    /* 
        TRC(); 
        if (strncmp(name, "agps", strlen(name)) == 0) { 
            return &rkAGpsInterface; 
        } 
        if (strncmp(name, "gps-ni", strlen(name)) == 0) { 
            return &rkGpsNiInterface; 
        } 
        if (strncmp(name, "agps_ril", strlen(name)) == 0) { 
            return &rkAGpsRilInterface; 
        } 
        if (strncmp(name, "supl-certificate", strlen(name)) == 0) { 
           return &rkSuplCertificateInterface; 
        } 
        if (strncmp(name, GPS_MEASUREMENT_INTERFACE, strlen(name)) == 0) { 
           return &rkGpsMeasurementInterface; 
        } 
        if (strncmp(name, GPS_NAVIGATION_MESSAGE_INTERFACE, strlen(name)) == 0) { 
           return &rkGpsNavigationMessageInterface; 
        }*/
    return NULL;
}

static const GpsInterface  rkGpsInterface = {
    sizeof(GpsInterface),
    rk_gps_init,
    rk_gps_start,
    rk_gps_stop,
    rk_gps_cleanup,
    rk_gps_inject_time,
    rk_gps_inject_location,
    rk_gps_delete_aiding_data,
    rk_gps_set_position_mode,
    rk_gps_get_extension,
};

const GpsInterface* gps__get_gps_interface(struct gps_device_t* dev) {
    DBG("gps__get_gps_interface HAL\n");
    return &rkGpsInterface;
}
